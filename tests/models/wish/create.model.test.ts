...
describe("Wish Create Model Test", () => {
    ...
    test("Test empty user name", async () => {
        const res = await wishCreateModel({ username: "", wish: mockWishModelInfo.wish }, mockWishModelInfo.wishes)
        expect(res.success).toBe(false)
        expect(res.error.message).toBe("[Name is required(zod)]")
    })

    test("Test empty wish", async () => {
        const res = await wishCreateModel({ username: mockWishModelInfo.username, wish: "" }, mockWishModelInfo.wishes)
        expect(res.success).toBe(false)
    ...
    test("Test Not register user", async () => {
        const res = await wishCreateModel({ username: mockWishModelInfo.username, wish: mockWishModelInfo.wish }, mockWishModelInfo.wishes)
        expect(res.success).toBe(false)
        expect(res.error.message).toBe("User is not registered")
    })

    test("Valid User", async () => {
        const res = await wishCreateModel({ username: "charlie.brown", wish: mockWishModelInfo.wish }, mockWishModelInfo.wishes)
        expect(res.success).toBe(true)
        expect(res.error.message).toBeNull()
    })

    test("Valid User with over 10 years old", async () => {
        const res = await wishCreateModel({ username: "james.bond", wish: mockWishModelInfo.wish }, mockWishModelInfo.wishes)
        expect(res.success).toBe(false)
